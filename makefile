...
JAVA_OPTS=" -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=de.lebenshilfe_muenster.uk_gebaerden_muensterland"
...
espresso-tests.log : app-original.apk app-androidTest.apk running-emulator
	...
	$(ADB) shell am instrument -w -r de.lebenshilfe_muenster.uk_gebaerden_muensterland.test/androidx.test.runner.AndroidJUnitRunner | tee espresso-tests.log
	export ABC_CONFIG=$(ABC_CFG) && $(ABC) stop-all-emulators
	rm running-emulator
...
%.testlog: app-androidTest.apk app-instrumented.apk running-emulator
	...
	export ABC_CONFIG=$(ABC_CFG) &&$(ABC) install-apk app-androidTest.apk
	$(ADB) shell am instrument -w -e class $(shell echo "$(@)" | tr "_" "#" | sed -e "s|.testlog||") de.lebenshilfe_muenster.uk_gebaerden_muensterland.test/androidx.test.runner.AndroidJUnitRunner | tee $(@)
	export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces de.lebenshilfe_muenster.uk_gebaerden_muensterland ./traces/$(shell echo "$(@)" | sed -e "s|.testlog||") force-clean
...
copy-shadows :
	cp -v ./shadows/*.java app/src/carvedTest/de/lebenshilfe_muenster/uk_gebaerden_muensterland
