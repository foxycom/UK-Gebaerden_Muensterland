...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=de.lebenshilfe_muenster.uk_gebaerden_muensterland "

INSTRUMENTATION_OPTS=" \
--skip-class=de.lebenshilfe_muenster.uk_gebaerden_muensterland.R \
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "de.lebenshilfe_muenster.uk_gebaerden_muensterland$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "de.lebenshilfe_muenster.uk_gebaerden_muensterland.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall de.lebenshilfe_muenster.uk_gebaerden_muensterland > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall de.lebenshilfe_muenster.uk_gebaerden_muensterland.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm clear de.lebenshilfe_muenster.uk_gebaerden_muensterland > /dev/null; \
fi)
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) de.lebenshilfe_muenster.uk_gebaerden_muensterland.test/androidx.test.runner.AndroidJUnitRunner 2>&1 | tee $(@)
#	Copy the traces if the previous command succeded
	@export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces de.lebenshilfe_muenster.uk_gebaerden_muensterland ./traces/$(TEST_NAME) force-clean
